...
void mg_mgr_init_opt(struct mg_mgr *m, void *user_data,
    ...
    {
        int i;
        if (opts.num_ifaces == 0) {  //传入参数opt.num_ifaces 为 0,初始化系统的接口

            printf("mg_mgr_init_opt the mg_num_ifaces[%d]\n", mg_num_ifaces);
            opts.num_ifaces = mg_num_ifaces;  //默认是2个接口
            opts.ifaces = mg_ifaces;  //{&mg_default_iface_vtable, &mg_tun_iface_vtable};
        }
        ...
        for (i = 0; i < mg_num_ifaces; i++) {
            ...
            m->ifaces[i]->vtable->init(m->ifaces[i]);  //系统默认是 mg_socket_if_init
        }
    }
    DBG(("=================================="));
    DBG(("init mgr=%p", m));
...
MG_INTERNAL struct mg_connection *mg_create_connection_base(
    ...
    if ((conn = (struct mg_connection *) MG_CALLOC(1, sizeof(*conn))) != NULL) {
         ...
         */
        // bruce.lu: for C1 Server, we don't expect too long a packet, 64K is sufficient.
        // conn->recv_mbuf_limit = ~0;
        conn->recv_mbuf_limit = 1024 * 64;
    } else {
...
/*
 * 通过struct mg_iface_vtable *,struct mg_mgr *　创建struct mg_iface *
 *
 */
